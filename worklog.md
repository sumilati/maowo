...
---
Task ID: login-fix-verify
Agent: Z.ai Code (main)
Task: 验证登录失败场景修复

Work Log:
- 复现各种登录/注册失败场景，确认错误提示均能显示
- 注册重复邮箱 → 红框"该邮箱已注册" + toast"登录失败 该邮箱已注册"
- 注册密码<6位 → "密码至少 6 位"
- 登录错误密码 → "邮箱或密码错误，请重试"
- 登录正确密码 → 成功跳主页
- 注册新邮箱 → 成功自动登录跳空状态主页
...
Stage Summary:
- 登录/注册各失败场景都有明确红框+toast 提示
- 之前用户"登录失败"最可能原因：密码少于6位、邮箱已注册、或密码输错，现在都有清晰提示
- demo 账号 demo@maowo.com/123456 可正常登录
