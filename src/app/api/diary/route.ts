...
export async function GET(req: NextRequest) {
  try {
    ...
    const list = await db.diary.findMany({ where: { catId }, orderBy: { date: 'desc' } })
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    // 校验归属：通过 diary 找到 catId 再校验
    const item = await db.diary.findUnique({ where: { id }, select: { catId: true } })
