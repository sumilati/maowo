...
export async function GET(req: NextRequest) {
  try {
    ...
    const list = await db.weightRecord.findMany({ where: { catId }, orderBy: { date: 'asc' } })
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (ok instanceof Response) return ok
    const item = await db.weightRecord.create({
      data: {
        catId: body.catId,
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const item = await db.weightRecord.findUnique({ where: { id }, select: { catId: true } })
